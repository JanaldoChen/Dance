...
class SMPL(nn.Module):
    def __init__(self, pkl_path, hresMapping_pkl_path='assets/hresMapping.pkl', rotate=False, isHres = False):
        ...
        self.faces_hres = torch.from_numpy(nf.astype(np.int32)).type(dtype=torch.int32)
        
        weights_hres = torch.FloatTensor(np.hstack([
            np.expand_dims(
                np.mean(
    ...
    def forward(self, beta, theta, v_personal=None, get_joints=False):
        """
        ...
        if self.isHres:
            W = self.weights_hres.repeat(num_batch, 1).view(num_batch, -1, 24)
        else:
        ...
        joint_x = torch.matmul(verts[:, :self.size[0], 0], self.joint_regressor)
        joint_y = torch.matmul(verts[:, :self.size[0], 1], self.joint_regressor)
        joint_z = torch.matmul(verts[:, :self.size[0], 2], self.joint_regressor)
